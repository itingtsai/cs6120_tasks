...
class CFGAnalyzer:
    """Control Flow Graph analyzer with dominance utilities (terminal output only)."""
    ...
    def __init__(self, func):
        ...
        self.idom: List[int | None] = []       # immediate dominator per block (None for entry)
        self.dom_tree: Dict[int, List[int]] = {}
        self.dom_frontier: Dict[int, Set[int]] = {}
        
        self._build_cfg()
        self._compute_dominators()
        self._build_dom_tree()       # also fills self.idom
    ...
    def _build_cfg(self):
        """Build basic blocks and control flow graph."""
        current = []
        current_label = None
    ...
    def _add_edge(self, from_idx, to_idx):
        """Add edge from block from_idx to block to_idx."""
    ...
    def _compute_dominators(self):
        ...
        if n > 0:
            self.dominators[0] = {0}  # entry dominates itself
        ...
        while changed:
            ...
            for i in range(1, n):
                ...
                if not preds:
                    # unreachable block: by convention, keep as all nodes (or just itself)
                    new_dom = {i}
                else:
                    new_dom = set(range(n))
                    for p in preds:
    ...
    def _build_dom_tree(self):
        """Build dominator tree and compute immediate dominators (idom)."""
        n = len(self.blocks)
        self.idom = [None] * n
        if n == 0:
            self.dom_tree = {}
        ...
        # Standard idom computation from dominator sets:
        # idom(b) = unique d in dom(b)\{b} that does not dominate any other candidate in dom(b)\{b}
        for b in range(1, n):
            candidates = self.dominators[b] - {b}
            idom_b = None
            for d in candidates:
                # d is idom if no other c (c != d, c != b) in candidates with d in dom(c)
                is_idom = True
                for c in candidates:
    ...
    def _compute_dom_frontier(self):
        """Compute dominance frontier using classic algorithm with idom."""
        n = len(self.blocks)
        self.dom_frontier = {b: set() for b in range(n)}
        # For each node with multiple predecessors, walk up the idom chain of each predecessor
        for b in range(n):
    ...
    def verify_dominators(self) -> bool:
        """Test dominators by checking intersection over all paths from entry."""
